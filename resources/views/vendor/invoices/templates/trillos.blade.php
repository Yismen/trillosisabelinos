...
<html lang="en">
    ...
    <body>
        ...
        @if (count($invoice->images) > 0)
        <p>
            <h5 style="margin-bottom: 30px;">Images</h5>
            <p>
                @foreach ($invoice->images as $image)
                <a href="{{ url('/storage/' . $image )}}" target="{{ $image }})" style="display: inline-block; margin-right: 15px;" >
                    <img src="{{ $invoice->getImage($image) }}" alt="{{ $image }}" height="100" />
                </a>
            @endforeach
            </p>
        </p>
